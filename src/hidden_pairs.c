            ...
            for (int j = i+1; j < num_value; j++){
                if(is_hidden_pair(p_cells, hidden_double_value_array[i], hidden_double_value_array[j])){
                    for (int k = 0; k < BOARD_SIZE; k++){
                        ...
                        HiddenPair hidden_double_object;
                        hidden_double_object.value1 = hidden_double_value_array[i];
                        hidden_double_object.value2 = hidden_double_value_array[j];
                        // hidden_double_object.p_cells = p_cells[k];
                        hidden_double_object.p_cells[i] = p_cells[k]; // where i is some valid index
...
int hidden_pairs(SudokuBoard *p_board)
{
    ...
    for (int i = 0; i < p_counter; i++){
        int not_overlap = 0;
        // Cell *p_cells = p_hidden_double[i].p_cells;
        // Cell (*p_cells)[BOARD_SIZE] = &p_hidden_double[i].p_cells; // pointer to an array
        Cell **p_cells = p_hidden_double[i].p_cells;
        ...
        for (int j = 0; j < num_candidates; j++){
            if ((candidate_array[j]!= p_hidden_double[i].value1)&&(candidate_array[j]!=p_hidden_double[i].value2)){
                // unset_candidate(p_cells, candidate_array[j]);
